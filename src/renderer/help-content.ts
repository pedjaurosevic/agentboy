...
export const HELP_SECTIONS: HelpSection[] = [
  {
    ...
    items: [
      ...
      { key: "FX", label: "Sweep / Noise / Chroma / Flicker / Vignette / Curve", desc: "Independent CRT extras that STACK on any mode except Off: Sweep rolls a retrace band down the tube; Noise adds broadcast grain; Chroma splits the picture into an RGB fringe; Flicker gives the brightness a restless flutter; Vignette sinks the corners into shadow; Curve reads the tube as a bulged glass dome. Combine freely — mode picks the mask, FX picks the life." },
      { key: "Wear", label: "New / Worn / Cracked / Glass", desc: "Chassis finish: pristine, grimy, cracked plastic, or fractured screen glass (scattered hairline cracks, any theme)." },
    ],
  },
  {
    ...
    items: [
      ...
      { key: "F4", label: "CRT", desc: "Cycle CRT mode (Shadow Mask → … → Off). Coming back from Off plays the old-tube turn-on flash. The LOOK menu FX row adds six independent toggles — Sweep, Noise, Chroma, Flicker, Vignette, Curve — that stack on any CRT mode except Off." },
      { key: "F5", label: "CRT−", desc: "CRT intensity down (20 levels)." },
      { key: "F6", label: "CRT+", desc: "CRT intensity up (20 levels)." },
      { key: "F7", label: "WEAR", desc: "Cycle New → Worn → Cracked → Glass. (This is the button at position F7; the physical F7 key is CRT intensity up — see Keyboard shortcuts.)" },
  ...
  },
  {
    ...
    items: [
      ...
      { key: "Ctrl+Shift+C / V", label: "Copy / paste", desc: "Just selecting text already copies it (to both the clipboard and the X11 primary selection). Ctrl+Insert also copies; Shift+Insert or middle-click pastes the primary selection. Every paste is sanitised — control bytes and stray paste terminators are stripped." },
      { key: "Ctrl+Shift+L", label: "Activity & checkpoints", desc: "Open the session log: approvals, auto-checkpoints, and rollbacks, plus this repo's checkpoint history. Select a checkpoint there to restore it (asks for a native confirmation first)." },
